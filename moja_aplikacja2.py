...
option1 = st.selectbox('Wybierz model',('Regresja liniowa','Regresja wielomianowa',
...
lokalizacja = st.selectbox('Lokalizacja',('Stare Miasto','Nowe Miasto','Grunwald','Wilda',
                                          'Jeżyce'))
...
if rynek=='pierwotny':
    ...
    wiek = st.selectbox('Przedział wieku mieszkania',('<0,10>','<11,30>','<31,50>',
                                        '<51,70>','<71,90>','<91,110>','<111,131>'))
if rynek=='pierwotny':
    rynek=0
    wiek=0
else:
...
#regresja liniowa
    if option1=='Regresja liniowa':
        ...
        predictionsreg = reg.predict([[powierzchnia,rynek,pietro,lokalizacja,
                                       wiek,pokoje]])
...
#drzewa
    if option1=='Drzewa decyzyjne':
        ...
        predictions_tree = tree.predict([[powierzchnia,rynek,pietro,lokalizacja,
                                          wiek,pokoje]])
...
#wielomian
    if option1=='Regresja wielomianowa':
        ...
        predictions_poly = poly.predict([[powierzchnia,rynek,pietro,lokalizacja,
                                          wiek,pokoje]])
...
#las losowy
    if option1=='Las losowy':
        ...
        predictions_forest = forest.predict([[powierzchnia,rynek,pietro,lokalizacja,
                                              wiek,pokoje]])
